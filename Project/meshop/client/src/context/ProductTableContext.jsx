...
const ProductTableContextProvider = ({children}) => {
  ...
  const fetchProducts = async (category) => {
		const url = `http://localhost:5000/api/products/${category != 'All' ? `category/${category}` : '' }`
		console.log('url : ' + url)
		try{
...
maxIndex = rows;
	...
	// },[slicedRowData])
  return (
    <ProductTableContext.Provider value={{productTableData,setProductTableData,fetchProducts,createPagination,slicedRowData}}>
        {children}
