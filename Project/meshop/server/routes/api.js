...
const {getAllUser,getUser,deleteUser,updateUser,logoutUser} = require('../controllers/user')
const {getAllProduct,getProductFromCategory,getProductFromId,addProduct,updateProduct,deleteProduct,uploadProductImage,searchProduct} = require('../controllers/product') 
const {addToCart,removeFromCart,getUserCart,getCartDetail} = require('../controllers/cart')

const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadDir = path.join(__dirname, `../images/products/${req.body.category === 'headphone' ? req.body.category+'s' : req.body.category}`);
  ...
  filename: (req, file, cb) => {
    ...
    req.body.image = `http://localhost:5000/products/images/${req.body.category === 'headphone' ? req.body.category+'s' : req.body.category}/${finalFileName}`
