  ...
  const products = getAllProductData();
	...
	if(products !== null && products !== undefined){
		...
		//  console.log(newProduct[1][0].category);
		 const newProductData = Object.fromEntries(newProduct.map(data => [data[1].category+'s',data]))
...
const uploadProductImage = async(req,res) =>{
		...
		console.log();
		const isExist = fs.existsSync(`./images/products/${req.body.category !== 'headphone' ? req.body.category : `${req.body.category}s`}${image}`)
		console.log(isExist)
		if(isExist){
...
const deleteProduct = async (req,res) => {
	...
	const updatedData = Object.fromEntries(filteredData.map(product => [product[0].category+'s',product]))
